...
Matrix createLocalK(int e,mesh &m){
    ...
    float tau, kappa, delta, lambda, psi, eta,J,D, Ae;
    
    //Componentes de K
    // [ A+K  G ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixW, matrixL, matrixD;
    ...
    //Preparando matrixW (La añadi para solucionar el labo)
    calculateBPrima(BPrima);
    delta = m.getParameter(DELTA);
    productRealMatrix((delta*Ae*-1)/(8*D*D), productMatrixMatrix(Betat, (productMatrixMatrix(Alphat, productMatrixMatrix(Alpha,BPrima,2,2,3), 2, 2, 3)), 6,2,3) , matrixW);

    //Preparando matrixG (En clase conocida simplemente como G)
    lambda = m.getParameter(DELTA);
    productRealMatrix((2*lambda*J)/(3*6*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixL);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixW,matrixL,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float psi_x = m.getParameter(PSIX);
    float psi_y = m.getParameter(PSIY);
    float J = calculateLocalJ(e,m);
    calculateGammaMatrix(g_matrix);
    zeroes(f,2);

   //Asumiendo Psi como vector porque sino no se puede obtener una matriz 6x1
    f.at(0) = psi_x;
    f.at(1) = psi_y;
    zeroes(b0,6);
